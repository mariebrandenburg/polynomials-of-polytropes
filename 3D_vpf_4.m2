W = {43, 60, 77, 43, 77, 60, 60, 77, 43, 77, 60, 43}; -- weight vector in cone of interest
...
x31*x32*x42 % I; -- equals -x21*x43^2
...
psi = rem_(x21*x43^2);
psi = -psi;
sub(psi, {a12=>43_QQ, a13=>60, a14=>77, a21=>43, a23=>77, a24=>60, a31=>60, a32=>77, a34=> 43, a41=> 77, a42=>60, a43=>43}) -- 671573 lattice points
...
psi = -(1/2)*a12*a13^2+(1/6)*a13^3+a12*a13*a14-(1/2)*a12*a14^2-(1/2)*a13*a14^2+(
      ...
      *a24^2-(1/2)*a21*a24^2+(1/6)*a24^3-(1/2)*a21*a31^2+(1/6)*a31^3-(1/2)*a12*
      a32^2-(1/2)*a31*a32^2+(1/3)*a32^3-(1/2)*a12^2*a34+a12*a14*a34-(1/2)*a14^2*
      ...
      )*a21*a34^2+a21*a31*a41-(1/2)*a21*a41^2-(1/2)*a31*a41^2+(1/3)*a41^3-(1/2)*
      a31^2*a42+a12*a32*a42+a31*a32*a42-(1/2)*a32^2*a42+a31*a41*a42-(1/2)*a41^2*
      a42-(1/2)*a12*a42^2-(1/2)*a31*a42^2+(1/6)*a42^3-(1/2)*a12^2*a43+a12*a13*
      ...
      )*a12+(1/3)*a13+(1/6)*a14+(1/2)*a21+(1/6)*a23+(1/3)*a24+(1/3)*a31+(1/6)*
      a32+(1/2)*a34+(1/6)*a41+(1/3)*a42+(1/2)*a43+1

---      

Polymake check:
$R = new Polytope(INEQUALITIES=>[[43, -1, 1, 0],[60, -1, 0, 1], [77, -1, 0,0],[43,1,-1,0],[77,0,-1,1],[60,0,-1,0],[60,1,0,-1],[77,0,1,-1],[43,0,0,-1],[77,1,0,0],[60,0,1,0],[43,0,0,1]]);
