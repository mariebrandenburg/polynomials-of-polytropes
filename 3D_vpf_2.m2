W = {22, 32, 42, 42, 32, 22, 32, 32, 32, 22, 42, 32}; -- weight vector in cone of interest
...
I = f(M) + L_A;
minimalPrimes M; -- (x14, x21, x34, x13, x32, x31, x23, x24, x12) is a minimal prime of I
x41*x42*x43 % I; -- equals x43^3
...
rem = p % I;
psi = rem_(x43^3);
sub(psi, {a12=>22_QQ, a13=>32, a14=>42, a21=>42, a23=>32, a24=>22, a31=>32, a32=>32, a34=> 32, a41=> 22, a42=>42, a43=>32}) -- 102335 lattice points
...
psi = -(1/2)*a12*a13^2+(1/6)*a13^3+a12*a13*a14-(1/2)*a12*a14^2-(1/2)*a13*a14^2+(
      ...
      2*a31-(1/2)*a24*a31^2+(1/6)*a31^3-(1/2)*a31^2*a32-(1/2)*a12*a32^2+(1/6)*
      a32^3-(1/2)*a12^2*a34+a12*a14*a34-(1/2)*a14^2*a34+a14*a24*a34+a24*a31*a34+
      a12*a32*a34+a31*a32*a34-(1/2)*a32^2*a34-(1/2)*a12*a34^2-(1/2)*a24*a34^2-(1
      /2)*a31*a34^2+(1/6)*a34^3-(1/2)*a21^2*a41+a21*a23*a41-(1/2)*a23^2*a41+a21*
      a31*a41-(1/2)*a31^2*a41+a31*a32*a41-(1/2)*a32^2*a41-(1/2)*a23*a41^2-(1/2)*
      a32*a41^2+a12*a32*a42+a32*a41*a42-(1/2)*a12*a42^2-(1/2)*a32*a42^2-(1/2)*
      a41*a42^2+(1/3)*a42^3-(1/2)*a12^2*a43+a12*a13*a43-(1/2)*a13^2*a43+a13*a23*
      ...
      /2)*a43^2+(1/2)*a12+(1/3)*a13+(1/6)*a14+(1/6)*a21+(1/3)*a23+(1/2)*a24+(1/3
      )*a31+(1/3)*a32+(1/3)*a34+(1/2)*a41+(1/6)*a42+(1/3)*a43+1

---      

Polymake check:
$R = new Polytope(INEQUALITIES=>[[22, -1, 1, 0],[32, -1, 0, 1], [42, -1, 0,0],[42,1,-1,0],[32,0,-1,1],[22,0,-1,0],[32,1,0,-1],[32,0,1,-1],[32,0,0,-1],[22,1,0,0],[42,0,1,0],[32,0,0,1]]);
print $R->N_LATTICE_POINTS; -- 102335