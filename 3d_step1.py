...
for line in sys.stdin:
    ...
    e = [vector([1,0,0,0]), vector([0,1,0,0]), vector([0,0,1,0]), vector([0,0,0,1])]
    indices = [(i,j) for i in range(4) for j in range(4) if not i==j]
    kleene = [[0]*4 for i in range(4)]
    for (i,j) in indices:
        objective = e[i] - e[j]
