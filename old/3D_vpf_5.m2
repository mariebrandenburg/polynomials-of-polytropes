W = {44, 60, 78, 44, 78, 60, 78, 60, 44, 60, 78, 44}; -- weight vector in cone of interest
...
x31*x32*x41 % I; -- equals -x21*x43^2
...
psi = rem_(x21*x43^2);
psi = -psi;
sub(psi, {a12=>44_QQ, a13=>60, a14=>78, a21=>44, a23=>78, a24=>60, a31=>78, a32=>60, a34=> 44, a41=> 60, a42=>78, a43=>44}) -- 698677 lattice points
 
---

psi = -(1/2)*a12*a13^2+(1/6)*a13^3+a12*a13*a14-(1/2)*a12*a14^2-(1/2)*a13*a14^2+(
      1/3)*a14^3-(1/2)*a13*a23^2-(1/2)*a21*a23^2+(1/3)*a23^3-(1/2)*a13^2*a24+a13
      *a14*a24-(1/2)*a14^2*a24+a13*a23*a24+a21*a23*a24-(1/2)*a23^2*a24-(1/2)*a13
      *a24^2-(1/2)*a21*a24^2+(1/6)*a24^3-(1/2)*a21*a31^2+(1/3)*a31^3-(1/2)*a31^2
      *a32-(1/2)*a12*a32^2+(1/6)*a32^3-(1/2)*a12^2*a34+a12*a14*a34-(1/2)*a14^2*
      a34-(1/2)*a21^2*a34+a14*a24*a34+a21*a24*a34-(1/2)*a24^2*a34+a21*a31*a34-(1
      /2)*a31^2*a34+a12*a32*a34+a31*a32*a34-(1/2)*a32^2*a34-(1/2)*a12*a34^2-(1/2
      )*a21*a34^2+a21*a31*a41-(1/2)*a31^2*a41+a31*a32*a41-(1/2)*a32^2*a41-(1/2)*
      a21*a41^2-(1/2)*a32*a41^2+(1/6)*a41^3+a12*a32*a42+a32*a41*a42-(1/2)*a12*
      a42^2-(1/2)*a32*a42^2-(1/2)*a41*a42^2+(1/3)*a42^3-(1/2)*a12^2*a43+a12*a13*
      ...
      )*a12+(1/3)*a13+(1/6)*a14+(1/2)*a21+(1/6)*a23+(1/3)*a24+(1/6)*a31+(1/3)*
      a32+(1/2)*a34+(1/3)*a41+(1/6)*a42+(1/2)*a43+1

---      

Polymake check:
$R = new Polytope(INEQUALITIES=>[[44, -1, 1, 0],[60, -1, 0, 1], [78, -1, 0,0],[44,1,-1,0],[78,0,-1,1],[60,0,-1,0],[78,1,0,-1],[60,0,1,-1],[44,0,0,-1],[60,1,0,0],[78,0,1,0],[44,0,0,1]]);
print $R->N_LATTICE_POINTS; -- 698677