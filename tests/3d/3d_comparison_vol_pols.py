...
with open("../../output/3d/3d_vol_pols.txt","r") as input_file_multi_vol:
...
R = PolynomialRing(QQ, ','.join(['a'+str(i)+str(j) for i in range(1,5) for j in range(1,5) if not i==j]))


for index in range(len(multi_vol)):
    f = R(multi_vol[index])
    P = lattice_polytope(coords[index])
    Vol = 6 * P.affine_hull().volume()
    w = [int(x) for x in weights[index][1:-1].split(',')]
    if index == 0:
        mode = 'w'
    else:
        mode = "a"
    with open("3d_output_comparison_vol.txt", mode) as output_file_vols:
